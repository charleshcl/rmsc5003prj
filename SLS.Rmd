...
GSLS <- function(price, I0, V0, alpha, beta, gamma, K, r, Imin, isShowPlot) {
  ...
  if (!missing(isShowPlot)) {
    vResults = matrix(rep(0,T), nrow=numPath)
  }
  ...
  if (!missing(isShowPlot)) {
    matplot(time,t(vResults),type="l")
  }
...
for (a in seq(from=0.5, to=1, by=0.5)) {
  for (b in seq(from=0.5, to=1, by=0.5)) {
    for (k in seq(from=1, to=2, by=1)) {
      ...
      simulatedResults <- rbind(simulatedResults, stimulatedRec)
    }
  }
}
