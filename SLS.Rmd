...
GSLS <- function(price, I0, V0, alpha, beta, gamma, K, r, Imin, isShowPlot, modelName) {
  ...
  pctReturn = matrix(rep(0,T*numPath), nrow=numPath)
  ...
  if (!missing(isShowPlot)) {
    sample = apply(pctReturn, 2, quantile, probs=c(0.01, 0.5, 0.99))
    plot(c(1:253, 1:253, 1:253), c(sample[1,],sample[2,],sample[3,]), main=paste("Contour of ", modelName, " alpha =", a, " beta =", b, " K=", k), xlab="time", ylab="pct return")
  }
  
  result = data.frame(gain3m    = g3mResult, 
                      acctVal3m = v3mResult, 
...
for (a in seq(from=0.5, to=1.5, by=0.5)) {
  for (b in seq(from=0.5, to=1.5, by=0.5)) {
    for (k in seq(from=1, to=3, by=1)) {
      # for simplicity, we set V_0 and I_0 for both stocks to be the same, for ease of computing porfolio return
      # rs stands for result set
      rsS1 <- GSLS(S1_gen, I0=I_0, V0=V_0, alpha=a, beta=b, gamma=g, K=k, r=dailyRiskFeeRate, Imin=I_min)
      rsS2 <- GSLS(S2_gen, I0=I_0, V0=V_0, alpha=a, beta=b, gamma=g, K=k, r=dailyRiskFeeRate, Imin=I_min)
      
      rsAAPL <- GSLS(Output_AAPL, I0=I_0, V0=V_0, alpha=a, beta=b, gamma=g, K=k, r=dailyRiskFeeRate, Imin=I_min)
      rsAMZN <- GSLS(Output_AMZN, I0=I_0, V0=V_0, alpha=a, beta=b, gamma=g, K=k, r=dailyRiskFeeRate, Imin=I_min)

      rsBEKK_AAPL <- GSLS(Output_BEKK_AAPL, I0=I_0, V0=V_0, alpha=a, beta=b, gamma=g, K=k, r=dailyRiskFeeRate, Imin=I_min)
      rsBEKK_AMZN <- GSLS(Output_BEKK_AMZN, I0=I_0, V0=V_0, alpha=a, beta=b, gamma=g, K=k, r=dailyRiskFeeRate, Imin=I_min)

      hist(rsS1$return1y+rsS2$return1y, main=paste("Histogram of GBM Histogram: alpha =", a, " beta =", b, " K=", k))
      hist(rsBEKK_AAPL$return1y+rsBEKK_AMZN$return1y, main=paste("Histogram of GARCH Histogram: alpha =", a, " beta =", b, " K=", k))
      
      # matplot(time1y,t(mean(rsS1$return1y+rsS2$return1y)),type="l")

      stimulatedRec <- data.frame(alpha=a,
                        beta=b,
                        K=k,
                        ### 3 mths
                        VaR99_3m_GBM_S1= getVaR(rsS1$return3m, pct99), 
                        VaR99_3m_GBM_S2=getVaR(rsS2$return3m, pct99), 
                        VaR99_3m_GBM_PTFL=getVaR(rsS1$return3m+rsS2$return3m, pct99), 
                        VaR99_3m_GARCH_S1=getVaR(rsAAPL$return3m, pct99), 
                        VaR99_3m_GARCH_S2=getVaR(rsAMZN$return3m, pct99), 
                        VaR99_3m_GARCH_PTFL=getVaR(rsBEKK_AAPL$return3m+rsBEKK_AMZN$return3m, pct99), 
                        ES99_3m_GBM_S1=getES(rsS1$return3m, pct99), 
                        ES99_3m_GBM_S2=getES(rsS2$return3m, pct99), 
                        ES99_3m_GBM_PTFL=getVaR(rsS1$return3m+rsS2$return3m, pct99), 
                        ES99_3m_GARCH_S1=getES(rsAAPL$return3m, pct99), 
                        ES99_3m_GARCH_S2=getES(rsAMZN$return3m, pct99), 
                        ES99_3m_GARCH_PTFL=getVaR(rsBEKK_AAPL$return3m+rsBEKK_AMZN$return3m, pct99),
                        ### 1 year
                        VaR99_1y_GBM_S1= getVaR(rsS1$return1y, pct99), 
                        VaR99_1y_GBM_S2=getVaR(rsS2$return1y, pct99), 
                        VaR99_1y_GBM_PTFL=getVaR(rsS1$return1y+rsS2$return1y, pct99), 
                        VaR99_1y_GARCH_S1=getVaR(rsAAPL$return1y, pct99), 
                        VaR99_1y_GARCH_S2=getVaR(rsAMZN$return1y, pct99), 
                        VaR99_1y_GARCH_PTFL=getVaR(rsBEKK_AAPL$return1y+rsBEKK_AMZN$return1y, pct99), 
                        ES99_1y_GBM_S1=getES(rsS1$return1y, pct99), 
                        ES99_1y_GBM_S2=getES(rsS2$return1y, pct99), 
                        ES99_1y_GBM_PTFL=getVaR(rsS1$return1y+rsS2$return1y, pct99), 
                        ES99_1y_GARCH_S1=getES(rsAAPL$return1y, pct99), 
                        ES99_1y_GARCH_S2=getES(rsAMZN$return1y, pct99), 
                        ES99_1y_GARCH_PTFL=getVaR(rsBEKK_AAPL$return1y+rsBEKK_AMZN$return1y, pct99)
                        )
      simulationResults <- rbind(simulationResults, stimulatedRec)
    }
  }
}
