...
for (a in seq(from=0.5, to=0.5, by=0.5)) {
  for (b in seq(from=0.5, to=0.5, by=0.5)) {
    for (k in seq(from=1, to=1, by=1)) {
      ...
      # ggplot(df, aes(x=gbmPftlRet3m)) +  geom_histogram( aes(y=..density..), color="black", fill="white") + geom_vline(aes(xintercept=mean(gbmPftlRet3m)), color="blue", linetype="dashed", size=1) + geom_density(alpha=.2, fill="#FF6666")
