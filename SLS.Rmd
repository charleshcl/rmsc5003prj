...
GSLS <- function(price, I0, V0, alpha, beta, gamma, K, r, Imin) {
  ...
  g10dayResult = rep(0,numPath)
  g30dayResult = rep(0,numPath)
  ...
  result = data.frame(gain10day = g10dayResult, 
                      gain30day = g30dayResult, 
...
for (a in seq(from=0.5, to=1.5, by=0.5)) {
  for (b in seq(from=0.5, to=1.5, by=0.5)) {
    for (k in seq(from=1, to=3, by=1)) {
      ...
      hist(g_S1$gain1year+g_S2$gain1year, main=paste("Histogram of GBM Histogram: alpha =", a, " beta =", b, " K=", k))
      hist(g_BEKK_AAPL$gain1year+g_BEKK_AMZN$gain1year, main=paste("Histogram of GARCH Histogram: alpha =", a, " beta =", b, " K=", k))

      reportCurrRec <- data.frame(alpha=a,
                        beta=b,
                        K=k,
                        VaR_GBM_S1= getVaR(g_S1$gain1year, VaR99pct), 
                        VaR_GBM_S2=getVaR(g_S2$gain1year, VaR99pct), 
                        VaR_GBM_PTFL=getVaR(g_S1$gain1year+g_S2$gain1year, VaR99pct), 
                        VaR_GARCH_S1=getVaR(g_AAPL$gain1year, VaR99pct), 
                        VaR_GARCH_S2=getVaR(g_AMZN$gain1year, VaR99pct), 
                        VaR_GARCH_PTFL=getVaR(g_BEKK_AAPL$gain1year+g_BEKK_AMZN$gain1year, VaR99pct), 
                        ES_GBM_S1=getES(g_S1$gain1year, VaR99pct), 
                        ES_GBM_S2=getES(g_S2$gain1year, VaR99pct), 
                        ES_GBM_PTFL=getVaR(g_S1$gain1year+g_S2$gain1year, VaR99pct), 
                        ES_GARCH_S1=getES(g_AAPL$gain1year, VaR99pct), 
                        ES_GARCH_S2=getES(g_AMZN$gain1year, VaR99pct), 
                        ES_GARCH_PTFL=getVaR(g_BEKK_AAPL$gain1year+g_BEKK_AMZN$gain1year, VaR99pct)
                        )
