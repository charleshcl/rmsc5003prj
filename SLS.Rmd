...
GSLS <- function(price, I0, V0, alpha, beta, gamma, K, r, Imin
  ...
  result = list(gain3m    = g3mResult, 
                      acctVal3m = v3mResult, 
                      return3m  = (v3mResult-V_0)/V_0, 
                      gain1y    = g1yResult, 
                      acctVal1y = v1yResult,
                      return1y  = (v1yResult-V_0)/V_0,
                      allV  = allV)
...
for (a in seq(from=0.5, to=0.5, by=0.5)) {
  for (b in seq(from=0.5, to=0.5, by=0.5)) {
    for (k in seq(from=1, to=1, by=1)) {
      ...
      # plot histogram
      hist( gbmPftlRet1y,   main=paste("Histogram of GBM Histogram: alpha =", a, " beta =", b, " K=", k))
      hist( garchPftlRet1y, main=paste("Histogram of GARCH Histogram: alpha =", a, " beta =", b, " K=", k))
      ...
      plot(c(1:253, 1:253, 1:253), c(sample[1,],sample[2,],sample[3,]), main=paste("Contour of Portfolio Simulation based on GBM Model", "alpha =", a, " beta =", b, " K=", k), xlab="time", ylab="pct return")
