...
GSLS_Ptfl <- function(price1, price2, I0, V0, alpha, beta, gamma, K, r, Imin) {
    ...
    for (t in 1:T) {
      ...
      if (t == 1) {
        ...
        if (ctrlrReset[1,t-1] >= Imin) {
          ...
          IS[1,t]  <- IS[1,1]
          I[1,t] <- 0
          gL[1,t] <- 0 # g function also need to be reset 
          gS[1,t] <- 0
          g[1,t]  <- gL[1,t] + gS[1,t]
        }
        if (ctrlrReset[2,t-1] >= Imin) {
          ...
          IS[2,t]  <- IS[2,1]
          I[2,t] <- 0
          gL[2,t] <- 0 # g function also need to be reset 
          gS[2,t] <- 0
          g[2,t]  <- gL[2,t] + gS[2,t]
        }
  ...
  time = (0:T)
  # message("vResults=", length(price))
  
  result = list(      acctVal3m = v3mResult, 
    ...
    for (t in 1:T) {
      ...
      if (t == 1) {
        ...
        if (ctrlrReset[t-1] >= Imin) {
          ...
          IS[t]  <- IS[1]
          I[t] <- 0
          gL[t] <- 0 # g function also need to be reset 
          gS[t] <- 0
          g[t]  <- gL[t] + gS[t]
        }
        # In a margin call, IL and IS need to be adjusted accordingly
...
#      garchPftlRet1y = rsBEKK_ptfl$return1y
      ...
      # ggplot(gbmPftlRet3m, aes(x=gbmPftlRet3m)) +  geom_histogram( aes(y=..density..), color="black", fill="white") + geom_vline(aes(xintercept=mean(gbmPftlRet3m)), color="blue", linetype="dashed", size=1) + geom_density(alpha=.2, fill="#FF6666")

           # ggplot(rsS1, aes(x=rsS1$return3m)) +  geom_histogram( aes(y=..density..), color="black", fill="white") + geom_vline(aes(xintercept=mean(rsS1$return3m)), color="blue", linetype="dashed", size=1) + geom_density(alpha=.2, fill="#FF6666")
      ...
      # plot contour
      sample = apply( (rsS2$allV + rsS2$allV - sum_V_0) / sum_V_0, 2, quantile, probs=c(0.01, 0.5, 0.99))
      ...
      sample = apply( (rsBEKK_AAPL$allV + rsBEKK_AMZN$allV - sum_V_0) / sum_V_0, 2, quantile, probs=c(0.01, 0.5, 0.99))
      plot(c(1:253, 1:253, 1:253), c(sample[1,],sample[2,],sample[3,]), main=paste("ontour of Portfolio Simulation based on GARCH Model", "alpha =", a, " beta =", b, " K=", k), xlab="time", ylab="pct return")
