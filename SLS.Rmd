...
## Global variables
```{r global variables}
P =500 # Number of Sample Path
# P = 10000 # Number of Sample Path
path = P
T = 63   # Length of simulation Series
T_1m = 21 # 252 / 12
T_3m = 63 # 252 / 4
# parameters
...
## Get Stock Data. Could skip if you want to use offline data we have downloaded
```{r get stock data}
price_AAPL = getSymbols("AAPL", from ="2015-11-15", to = "2018-11-15", auto.assign = FALSE)
...
## price: n by t matrix
##   n: number of paths
##   t: must be 63 days, which is three months trading days
...
# to work offline. comment this section if you want to get data online
...
# get offline stock data. toggle comment with the above section if you want online data
rm(price_AAPL, price_AMZN)
load(file = "StockPriceData.rda")
S1 = price_AAPL
S2 = price_AMZN
...
# get offline data. toggle with above part
AAPL <- as.numeric(price_AAPL$AAPL.Close)
AMZN <- as.numeric(price_AMZN$AMZN.Close)
...
simulationResults <- data.frame(alpha=double(),
                        beta=double(),
                        K=double(),
                        VaR99_1m_GBM_S1=double(), 
                        VaR99_1m_GBM_S2=double(), 
                        VaR99_1m_GBM_PTFL=double(), 
                        VaR99_1m_GARCH_S1=double(), 
                        VaR99_1m_GARCH_S2=double(), 
                        VaR99_1m_GARCH_PTFL=double(), 
                        ES99_1m_GBM_S1=double(), 
                        ES99_1m_GBM_S2=double(), 
                        ES99_1m_GBM_PTFL=double(), 
                        ES99_1m_GARCH_S1=double(), 
                        ES99_1m_GARCH_S2=double(), 
...
#       gbmPftlRet1m   = (rsS1$acctVal1m+rsS2$acctVal1m-sum_V_0)/sum_V_0
#       garchPftlRet3m = (rsBEKK_AAPL$acctVal3m+rsBEKK_AMZN$acctVal3m-sum_V_0)/sum_V_0
#       garchPftlRet1m = (rsBEKK_AAPL$acctVal1m+rsBEKK_AMZN$acctVal1m-sum_V_0)/sum_V_0
      
      # plot histogram and denity curve
      hist( gbmPftlRet1m,   main=paste("Histogram of Portfolio Return under BS Model with \n alpha =", a, " beta =", b, " K=", k, "T = 1 Month"), xlim =c(min(gbmPftlRet1m)-0.1, max(gbmPftlRet1m)+0.1),breaks = 20, xlab="Return")
      dens=density(gbmPftlRet1m)
      lines(dens$x,length(gbmPftlRet1m) * (max(gbmPftlRet1m) - min(gbmPftlRet1m)) *dens$y / 20) 
      hist( garchPftlRet1m, main=paste("Histogram of Portfolio Return under GARCH Model with \n alpha =", a, " beta =", b, " K=", k, "T = 1 Month"), xlim =c(min(garchPftlRet1m)-0.1, max(garchPftlRet1m)+0.1),breaks = 20, xlab="Return")
      dens=density(garchPftlRet1m)
      lines(dens$x,length(garchPftlRet1m) * (max(garchPftlRet1m) - min(garchPftlRet1m)) *dens$y / 20) 
      
      hist( gbmPftlRet3m,   main=paste("Histogram of Portfolio Return under BS Model with \n alpha =", a, " beta =", b, " K=", k, "T = 3 Months"), xlim =c(min(gbmPftlRet3m)-0.1, max(gbmPftlRet3m)+0.1),breaks = 20, xlab="Return")
      ...
      plot.zoo(t( (rsBS_ptfl$allV - sum_V_0) / sum_V_0 ), main = paste("Sample Return Paths and Empirical Confidence Envelope (BS Model)\n", "alpha =", a, " beta =", b, " K=", k), xlab="t", ylab="Portfolio Return", plot.type = "single")
      ...
      plot.zoo(t((rsBEKK_ptfl$allV - sum_V_0) / sum_V_0), main = paste("Sample Return Paths and Empirical Confidence Envelope (BEKK Model)\n", "alpha =", a, " beta =", b, " K=", k), xlab="t", ylab="Portfolio Return", plot.type = "single")
      lines(sample[1,],col="Red",lwd=2)
      lines(sample[2,],col="Red",lwd=2)
      lines(sample[3,],col="Red",lwd=2)
      ...
      stimulationRec <- data.frame(alpha=a,
                        ...
                        ### 1 month
                        VaR99_1m_GBM_S1    =getVaR(rsS1$return1m, pct99), 
                        VaR99_1m_GBM_S2    =getVaR(rsS2$return1m, pct99), 
                        VaR99_1m_GBM_PTFL  =getVaR(gbmPftlRet1m, pct99), 
                        
                        VaR99_1m_GARCH_S1  =getVaR(rsAAPL$return1m, pct99), 
                        VaR99_1m_GARCH_S2  =getVaR(rsAMZN$return1m, pct99), 
                        VaR99_1m_GARCH_PTFL=getVaR(garchPftlRet1m, pct99), 
                        
                        ES99_1m_GBM_S1     =getES(rsS1$return1m, pct99), 
                        ES99_1m_GBM_S2     =getES(rsS2$return1m, pct99), 
                        ES99_1m_GBM_PTFL   =getES(gbmPftlRet1m, pct99), 
                        
                        ES99_1m_GARCH_S1   =getES(rsAAPL$return1m, pct99), 
                        ES99_1m_GARCH_S2   =getES(rsAMZN$return1m, pct99), 
...
length(price_AAPL) # expected 63

S_AAPL = matrix(price_AAPL, ncol=63)
S_AMZN = matrix(price_AMZN, ncol=63)
